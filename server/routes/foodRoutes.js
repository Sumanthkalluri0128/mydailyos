...
router.get('/',async(req,res)=>{const filter={};if(req.query.search)filter.$or=[{name:{$regex:req.query.search,$options:'i'}},{brand:{$regex:req.query.search,$options:'i'}}];if(req.query.favorites==='true')filter.isFavorite=true;res.json({success:true,foods:await Food.find(filter).sort({name:1}).lean()});});
